var TablaLugares = function () {
    ...
    return {
        ...
        init: function () {
			...
			$('#borrarlugar').live('click', function (e) {
				e.preventDefault();

				if (confirm("Seguro que desea eliminar el lugar y todos sus datos?") == false) {
					return;
				}
				
				var idlugar = $(this).attr("lugar-id");
				$.ajax({
					type:"POST",
					url: "borrarlugares.php",
					data:{id:idlugar}
				}).done(function(msg){
					if (msg == 0){
						//alert("Lugar borrado.");
